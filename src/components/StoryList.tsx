                ...
                {({ open }) => {
                    return (
                        <div className='flex flex-col items-center gap-2 cursor-pointer relative'>
                            <Image src={img?.secure_url || user?.imageUrl || "/noAvatar.png"} alt="" width={80} height={80} className="w-20 h-20 rounded-full ring-2" onClick={() => open()}/>
                            {img ? (<form action={add}>
                                <button className='text-xs bg-blue-500 p-1 rounded-lg text-white'>Send</button>
                            </form>) : (<span className='font-medium'>Add a Story</span>)}
                            <div className='absolute text-6xl top-1 text-gray-100'>+</div>
                        </div>
                    );
                }}
        </CldUploadWidget>
